...
export const extractPullRequestURLs = async (pullRequests, branch, githubUsername) => {
  console.log(await pullRequests);
...
const getPullRequestURLs = async (repo) => {
  ...
  return remotes.map(async (remoteUrl) => {
    ...
    return extractPullRequestURLs(pullRequests, branch, githubUsername);
...
const viewPullRequests = async () => {
  ...
  const pullRequestURLs = await getRepositories().map(getPullRequestURLs).reduce(concatArrayAsync, []);
